...
class botmanager(minqlx.Plugin):
    def __init__(self):
        ...
        self.set_cvar_once("bot_autoManage", "0", 4)
        # is a /serverinfo cvar so VSP can query it/stats services get it
    ...
    def bots_present(self):
        """
        returns true if bots are on the server, false otherwise
        """
    ...
    def bot_checks(self, flags):
        if "addbot" in flags.lower():
            if not self.get_cvar("bot_enable", bool):
                return (False, "^1Error:^7 Bots are not enabled on this server.")  # noqa
            if self.botError:
                return (False, "^1Error:^7 Bots are not supported on this map.")  # noqa
            if self.bots_present():
                return (False, "^1Error:^7 There is already a bot on this server.")  # noqa
            return (True, None)
        elif "rembot" in flags.lower():
            if not self.bots_present():
                return (False, "^1Error:^7 There is no bot currently on this server.")  # noqa
            return (True, None)
        return (False, None)

    def addbot(self):
        minqlx.console_command("addbot {} {} A 0 \"{}\"".format(BOT_TYPE[0], BOT_TYPE[1], BOT_NAME))  # noqa
        # team any, 0 millisecond join delay

    def rembot(self):
        minqlx.console_command("clientkick {}".format(self.find_player(BOT_NAME)[0].id)) # kicks BOT_NAME  # noqa
    ...
    def handle_map(self, mapname, factory):
        if self.get_cvar("bot_enable", bool):
            ...
            def f():
                if self.botError:
                    self.msg("^3Warning:^7 Bots are not supported on this map.")  # noqa
                    self.talk_beep()
            f()
    ...
    def handle_userinfo(self, player, changed):
        ...
        if "name" in changed:
            ...
            name = name.replace(ZERO_WIDTH_SPACE, "")
            if self.clean_text(BOT_NAME).lower() in name:
                player.kick("{} is a name reserved for bots. Please pick another name and re-connect.".format(self.clean_text(BOT_NAME)))  # noqa
